...
def test_user_data(web_client, test_web_address):
    ...
    response = web_client.get(f"http://{test_web_address}/profiles/data", data={
        "token": token_mock
    })
    ...
    assert response.status_code == 200
    assert response.get_json().get('message') == "OK!"
    assert response.get_json().get('users')[0] == {"email":"amina@gmail.com","id":1,"password":"amina1","username":"amina"}
...
def test_requests_null(web_client, test_web_address):
    ...
    response = web_client.get(f"http://{test_web_address}/requests/null", data={
        "token": token_mock
    })

    assert response.status_code == 200
    assert response.get_json().get('message') == "OK!"
    assert response.get_json().get('requests')[0] == {'id': 1, 'request_from': 1, 'request_to': 2, 'status': None}
...
def test_requests_null(web_client, test_web_address):
    ...
    response = web_client.get(f"http://{test_web_address}/requests/true", data={
        "token": token_mock
    })

    assert response.status_code == 200
    assert response.get_json().get('message') == "OK!"
    assert response.get_json().get('requests')[0] == {'id': 2, 'request_from': 2, 'request_to': 1, 'status': True}
...
def test_profiles_data(web_client, test_web_address):
    ...
    response = web_client.get(f"http://{test_web_address}/profiles/user_id", data={
        "token": token_mock,
        "id": 1
    })
    ...
    assert response.status_code == 200
    assert response.get_json().get('message') == "OK!"
    assert response.get_json().get('profile') == {'email': 'amina@gmail.com', 'id': 1, 'password': 'amina1', 'username': 'amina'}
    assert response.get_json().get('preferences') == {'age_slot': '[18, 24]', 'category': 'resort', 'continent': 'North America', 'gender': 'other', 'id': 1, 'season': 'winter', 'user_id': 1}
