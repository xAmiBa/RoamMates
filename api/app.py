...
def user_signup():
    ...
    if users.find_by_email(email) == []:
        ...
        response = app.response_class(
            response=json.dumps({"message": "OK!"}), status=200
        )

    else:
        print("User already exists:")
        response = app.response_class(
            response=json.dumps({"message": "Credentials error"}), status=401
        )
...
def user_login():
    ...
    if users_repo.check_login_details(email, password):
        user = users_repo.find_by_email(email)
        token = token_generator(user.id)
        response = app.response_class(
            response=json.dumps({"message": "OK!", "token": token}), status=200
        )
        return response
    return app.response_class(
        response=json.dumps({"message": "Invalid credentials"}), status=401
    )
